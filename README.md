# Graph Algorithms on a Randomly Generated City Network
...
Cities are represented as nodes, with roads as weighted, undirected edges. Each city is randomly connected to a handful of others, and the resulting network is analyzed for its structure and connectivity.

## Concepts Explored

### 1. Random Graph Construction
A graph is built where:
- Each node is given a random degree, controlling how many roads connect it to other cities.
- Edges carry random weights, representing road distance or cost.
...
- Looking at all components (their count, sizes, largest, and smallest) reveals whether the network is fully connected (a single component spanning all cities) or fragmented into isolated clusters — an important property when reasoning about network resilience and reachability.
